 ...
 */
export const detectPackageManager = (options: DetectOptions = {}): PackageManagerInfo => {
  ...
  let packageManagerDef: string | null = null;
  ...
  try {
    ...
    console.log(
      chalk.yellow('Error checking for lock files, continuing with default package manager.')
    );
  }
  ...
  try {
        ...
        if (fs.existsSync(parentPackageJsonPath)) {
          try {
            ...
            if (parentPackageJson.packageManager) {
              ...
              console.log(
                chalk.yellow(`This may cause conflicts. Consider using --dryRun option.`)
              );
    ...
    console.log(
      chalk.yellow('Could not read package.json, continuing with detected package manager.')
    );
  }
 ...
 */
export const buildInstallCommand = (options: BuildCommandOptions): string => {
  const {
    dependencies = [],
    packageManager = 'yarn',
    useCorepack = false,
  ...
  if (useCorepack && packageManagerDef) {
    ...
    console.log(
      chalk.blue(`Project uses ${corepackManager}@${corepackVersion} defined in package.json`)
    );
