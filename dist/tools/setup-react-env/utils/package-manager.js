...
const detectPackageManager = (options = {}) => {
  ...
  try {
    ...
    if (hasYarnLock && !hasNpmLock) {
      ...
      console.log(
        chalk_1.default.blue('Detected package-lock.json file, using npm as package manager')
      );
    } else if (hasYarnLock && hasNpmLock) {
      console.log(
        chalk_1.default.yellow(
          'Both yarn.lock and package-lock.json files detected. Using specified package manager.'
        )
      );
    }
  } catch (error) {
    console.log(
      chalk_1.default.yellow(
        'Error checking for lock files, continuing with default package manager.'
      )
    );
  }
  ...
  try {
    ...
    if (fs_1.default.existsSync(packageJsonPath)) {
      ...
      if (packageJson.packageManager) {
        ...
        console.log(
          chalk_1.default.blue(`Found packageManager: ${packageManagerDef} in ${packageJsonPath}`)
        );
      }
    }
        ...
        if (fs_1.default.existsSync(parentPackageJsonPath)) {
          try {
            const parentPackageJson = JSON.parse(
              fs_1.default.readFileSync(parentPackageJsonPath, 'utf8')
            );
            if (parentPackageJson.packageManager) {
              console.log(
                chalk_1.default.yellow(
                  `Warning: Found packageManager field in parent directory: ${currentDir}`
                )
              );
              console.log(
                chalk_1.default.yellow(`This may cause conflicts. Consider using --dryRun option.`)
              );
    ...
    console.log(
      chalk_1.default.yellow(
        'Could not read package.json, continuing with detected package manager.'
      )
    );
  }
  return {
  ...
  if (useCorepack && packageManagerDef) {
    ...
    console.log(
      chalk_1.default.blue(
        `Project uses ${corepackManager}@${corepackVersion} defined in package.json`
      )
    );
    ...
    console.log(
      chalk_1.default.blue(`Installing dependencies with ${corepackManager} (via npx)...`)
    );
  } else {
